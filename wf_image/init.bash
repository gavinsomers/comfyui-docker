...
##
echo ""; echo "== Update ComfyUI to latest version (if upgrades enabled)"
cd ComfyUI
if [ "A${DISABLE_UPGRADES}" == "Atrue" ]; then
  echo "== ComfyUI update disabled by DISABLE_UPGRADES"
else
  AUTO_UPDATE_COMFYUI=${AUTO_UPDATE_COMFYUI:-"true"}
  if [ "A${AUTO_UPDATE_COMFYUI}" == "Atrue" ]; then
    echo "== Pulling latest ComfyUI from git"
    git pull || echo "!! Warning: Failed to update ComfyUI, continuing with existing version"
  else
    echo "== ComfyUI auto-update disabled by AUTO_UPDATE_COMFYUI"
  fi
fi
cd ..
...
if [ "A${DISABLE_UPGRADES}" == "Atrue" ]; then
  ...
  ${PIP3_CMD} "huggingface_hub[cli]<1.0" || error_exit "HuggingFace Hub CLI install/upgrade failed"
fi

export COMFYUI_PATH=`pwd`
echo ""; echo "-- COMFYUI_PATH: ${COMFYUI_PATH}"

# Note: ComfyUI Manager is now installed as a Python package via manager_requirements.txt
# The Docker image's init script handles this automatically

# Update custom nodes directory
...
##
echo ""; echo "== Update custom nodes (if upgrades enabled)"
if [ "A${DISABLE_UPGRADES}" == "Atrue" ]; then
  echo "== Custom nodes update disabled by DISABLE_UPGRADES"
else
  AUTO_UPDATE_COMFYUI=${AUTO_UPDATE_COMFYUI:-"true"}
  if [ "A${AUTO_UPDATE_COMFYUI}" == "Atrue" ]; then
    echo "== Updating all git-based custom nodes"
    for node_dir in */; do
      if [ -d "${node_dir}.git" ]; then
        node_name=$(basename "$node_dir")
        echo "  -- Updating ${node_name}"
        cd "$node_dir"
        git pull || echo "  !! Warning: Failed to update ${node_name}, continuing"
        cd ..
      fi
    done
  else
    echo "== Custom nodes auto-update disabled by AUTO_UPDATE_COMFYUI"
  fi
fi

# Note: ComfyUI-Manager security level configuration is now handled by the Docker image's init script
# To configure security level, use the SECURITY_LEVEL environment variable in docker-compose.yaml
